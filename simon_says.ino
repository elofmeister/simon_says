...
uint8_t input = 0;
uint8_t spiral_cnt = 0; // counting spiral frames
...
void toggleSpiralAnimation(bool restartIfFinished = false)
{
  if (spiral_cnt++ < sizeof(SPIRAL) / sizeof(SPIRAL[0]) - 1)
  {
    setLedMatrix(SPIRAL[spiral_cnt]);
  }
  else
  {
    if (spiral_cnt % 2)
    {
      setLedMatrix(SPIRAL[0]);
    }
    else
    {
      setLedMatrix(SPIRAL[sizeof(SPIRAL) / sizeof(SPIRAL[0]) - 1]);
    }
