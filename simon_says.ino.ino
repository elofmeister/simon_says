...
// uint8_t player_lifes[][]; //keeps track of the number of turrs and the specific number in sequence
const uint8_t STARTING_LIFES = 3;
const unsigned int GAMESPEED = 3000;  //speed of the numbers shown
long player_life_turns[9][STARTING_LIFES];
long simon_said[50];
    ...
    if (irrecv.decode(&results))
         {          
           ...
           {                
              ...
              input_count++;
              player_life_turns[turn_player][2]++;    //correct etrys counted for highscore      
           }
           else 
           {
              lcd.print("wrong!!!");
...
void soft_reset() 
{
 delay(8001); //if delay is greater then 8seconds , immidiate wathcdog reset
}
...
void set_input() //returns the pressed input
{
  switch(results.value)
  {
    ...
    case 0xFF52AD: input=9;
                   break; 
    case 0xFFFFFF: break;   //if button is hold  
    default:  break; 
  }
}
void lcd_refresh() 
